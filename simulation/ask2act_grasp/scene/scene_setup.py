...
class SceneSetup:
  ...
  <worldbody>
    ...
    <body name="target_cup" pos="{cup_x} {cup_y} {cup_z}">
      ...
      <geom name="target_cup_body" type="cylinder" size="{cup_radius} {cup_half_height}" mass="{self.scene_config.cup_mass_kg * 0.65}" friction="{friction}" condim="6" rgba="0.82 0.20 0.16 1"/>
      <geom name="target_cup_base" type="cylinder" pos="0 0 {-cup_half_height + 0.008}" size="{cup_radius * 0.95} 0.008" mass="{self.scene_config.cup_mass_kg * 0.35}" friction="6.0 0.25 0.10" condim="6" rgba="0.70 0.12 0.10 1"/>
      <geom name="target_cup_lid" type="cylinder" pos="0 0 {cup_half_height - 0.006}" size="{cup_radius * 0.92} 0.006" mass="0.005" friction="{friction}" condim="6" rgba="0.94 0.92 0.85 1"/>
    </body>
  </worldbody>
</mujoco>
"""
